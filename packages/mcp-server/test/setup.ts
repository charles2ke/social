 ...
 * `packages/db/test/setup.ts` so `packages/mcp-server` can be tested
 * against a bare Postgres instance with no hand-provisioned database or
 * seed data.
 */
...
const schemaName = `test_mcp_${randomBytes(6).toString("hex")}`;
...
// `getPrismaClient()` (used by `packages/mcp-server`'s `server.ts`) is a
