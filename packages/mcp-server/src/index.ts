...
server.tool("update_draft", "Update a post draft.", { id: z.string(), text: z.string().min(1).optional(), mediaUrls: z.array(z.string().url()).optional(), media: mediaSchema }, async ({ id, ...changes }) => { const draft = drafts.get(id); if (!draft) return text({ error: "Draft not found" }); const updated: PostDraft = { ...draft }; if (changes.text !== undefined) updated.text = changes.text; if (changes.mediaUrls !== undefined) updated.mediaUrls = changes.mediaUrls; if (changes.media !== undefined) updated.media = changes.media; drafts.set(id, updated); return text({ id, ...updated }); });
